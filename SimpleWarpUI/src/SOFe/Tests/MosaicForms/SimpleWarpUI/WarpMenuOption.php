...
class WarpMenuOption extends MenuOption{
	/** @var Warp */
	private $warp;

	public function __construct(Warp $warp, Player $player){
		parent::__construct($warp->getName() .
			($player->hasPermission(SimpleWarpPermissions::LIST_WARPS_COMMAND_XYZ) ?
				(" (" . $warp->getDestination()->toString() . ")") : ""));
		$this->warp = $warp;
	}
